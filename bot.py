...
# file paths
jsonPath = "/root/housebot/reminders.json"
tokenPath = "/root/housebot/token.txt"
configPath = "/root/housebot/configu.txt"
mapsKeyPath = "/root/housebot/mapskey.txt"
...
async def on_ready():
    ...
    # checks if the JSON is greater than 2 bytes
    # (ie has data other than an empty list)
    if path.getsize(jsonPath) > 2:
        with open(jsonPath) as j:
            ...
            for x in reminderData:
                # if the JSON element's time is before or at the current time
                # send the reminder now with a message at the beginning
                ...
                # if the JSON element's time is after the current time
                # add it to a list to be run in the background task later
                elif int(x["time"]) > int(time.time()):
                    waitBool = True
                    waitForList.append(x)

        # if there are any elements of the JSON
        # whose time is after the current time
        if(waitBool):
...
# background task that is only run once for the reminders
# that are backed up in the JSON but have not happened yet
@tasks.loop(count=1)
...
async def info(ctx):
    ...
    embed.add_field(name="View source code:", value="https://github.com/TheGrimlessReaper/HouseBot", inline=False)
    await ctx.send(content=None, embed=embed)
...
# prints when the bot has been disconnected (checks every minute)
@bot.event
...
def getHourly(lat, lon, len):
    ...
    for f in hourlyForecasts['properties']['periods']:
        # datetime object of the time of the spot being iterated
        weatherTime = datetime.datetime.strptime(f['startTime'], "%Y-%m-%dT%H:%M:%S%z")
        # checks if the string of current hour in gmt is equal
        # to the string of the hour of the place in the forecast
        # converted to gmt (so it will work with any time zone)
    ...
    # embed string needs to be less than 1024 characters
    # because of a limitation with the Discord API
...
def getDaily(lat, lon, len):
    ...
    for f in dailyForecasts['properties']['periods']:
        # converting the datetime object we are iterating at
...
def getAlerts(lat, lon, desc):
    ...
    for f in alerts['features']:
        ...
        if(f['properties']['ends'] is not None):
        ...
        if(f['properties']['effective'] is not None):
        ...
        currentT = datetime.datetime.now().timestamp()
        # if the current time is before the end time
        # and after the start time of the alert
...
async def weather(ctx, *args):
    if args:
        async with ctx.channel.typing():
            ...
            # there will be nothing in args if the user types help
            # which will throw an error otherwise
            if(args):
            ...
            # forecast summary if no other args
            else:
